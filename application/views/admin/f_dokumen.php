...
<?php
	...
	echo $this->session->flashdata("k");
	 ...
	 $jfiles		= $this->db->query("select * from t_fileslks where nip='$nip'")->row();
...
<?php
		...
		$jpen		= $this->db->query("select * from a_docu where klas='".$klas."' and slks='1' order by urut")->result();
		...
		?>
				<form action="<?=base_URL()?>admin/slks/act_upload" method="post" accept-charset="utf-8" enctype="multipart/form-data">
				
					<tr><td width="30%"><?php echo $nama_file; ?> </td><td><b><input type="file" name="<?php echo $kd_file; ?>" class="form-control"></b></td>
	<?php
        $jfiles		= $this->db->query("select * from t_fileslks where nip='".$nip."' and id_doc='".$id_doc."'")->row();
		$jfile=json_decode(json_encode($jfiles), True);
		if(isset($jfiles)){
...
//			$tahun = isset($jfile['tahun'])?$jfile['tahun']:'';
	...
	<td width="5%"><button type="submit" class="btn btn-info">Upload</button></td>
		...
		if($idc == $id_doc && $idc != ''){	
				echo "<td><i><a href='".base_URL()."upload/slks/".$nip."/".$n_file."' target='_blank' class='btn btn-success active' role='button' aria-pressed='true'>Cek File</a></div>";
